...
class Parsedown extends ParsedownExtra
{
    ...
    protected function blockHeaders($Line)
    {
    ...
    function contentsList($res = '')
    {
        if ('string' === strtolower($res)) {
            $result = '';
            if (! empty($this->contentsListString)) {
                $result = $this->text($this->contentsListString);
            }
            return $result;
        } elseif ('json' === strtolower($res)) {
    ...
    protected function blockHeader($Line)
    {
        if (isset($Line['text'][1])) {
            ...
            $level = str_replace('h','',$Block['element']['name']);    //h1,h2..h6
